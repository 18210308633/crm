...
(function(a) {
	$.fn.extend({
		whjPaging: function(b, j, c) {
				...
				if (b === "setPage") {
					...
					if (g.get(0).pageText == null) {
						...
						g.get(0).pageText = {
							...
							totalNumText: b.totalNumText != null ? b.totalNumText: "共{}条记录",
							...
							isShowTotalNum: b.isShowTotalNum == false ? false: true,
							callBack: b.callBack,
							currPage: 1,
			...
			var q = '<div class="' + g.get(0).pageText.css + '" style="width:70%;">';
			...
			if (g.get(0).pageText.isShowPageSizeOpt) {
				q += ' <select class="whj_border whj_bgc whj_hover" name="whj_pageSize" style="width: 20%;">';
			...
			if (g.get(0).pageText.isShowTotalNum) {
				var d = g.get(0).pageText.totalNumText.replace("{}", " "+g.get(0).pageText.totalNum+" ");
				q += ' <div class="whj_padding whj_color">' + d + "</div>"
			}
			q += "</div>";
			g.html(q);
			if (g.get(0).pageText.isShowFL) {
			...
			if (g.get(0).pageText.isShowSkip) {
				g.find("input[name='whj_toPage']").on("input",
				function() {
					var v = $(this).val();
